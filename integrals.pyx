...
cdef double f_interp(double x, np.ndarray[np.float64_t, ndim = 1] x_vals, np.ndarray[np.float64_t, ndim = 1] y_vals):
    return np.interp(x, x_vals, y_vals)




def spp_1d_interp_integrate(np.ndarray matrix,  np.ndarray rm, np.ndarray x_vals, np.ndarray y_vals):
    ...
    for i in range(matrix.shape[0]):
        for j in range(matrix.shape[1]):
            ...
            result[i, j] = integrate.quad(lambda x: f_interp(x, x_vals, y_vals) * f_interp(x, x_vals, y_vals), lim_low, lim_up)[0]
    return result
